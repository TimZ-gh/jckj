...
function renderSubmitStep() {
  ...
  return el("div", { class: "section-stack" }, [
    sectionTitle("提交与导出", "提交后会保存在当前浏览器中，也可以导出 JSON / CSV。若配置了后端接口，会同步提交到服务器。"),
    el("div", { class: "summary-grid" }, [
      ...
      summaryItem("已收集提交", `${submissions.length} 条`),
    ]),
...
async function submitResponse() {
  ...
  alert("已提交。你可以继续导出 JSON / CSV，或刷新后填写下一份。");
  render();
}
...
function downloadCsv() {
  ...
  const lines = [
    ...
    ...rows.map((row) => [
      row.submittedAt,
      row.meta?.name,
      row.summary?.departmentLabel,
      row.meta?.role,
      (row.meta?.platforms || []).join(" / "),
      row.summary?.stepCount,
      (row.ai?.usefulAreas || []).join(" / "),
      (row.ai?.humanOnly || []).join(" / "),
      row.attachments?.links,
    ].map(csvCell).join(",")),
  ];
