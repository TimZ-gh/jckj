...
export const CRITICAL_HUMAN_OPTIONS = [
  "产品卖点确认",
  "价格 / 库存确认",
  "Listing 最终审核",
  "视频脚本审核",
...
export const COMMON_QUESTIONS = [
  {
    id: "main_responsibility",
    label: "你当前主要负责哪些工作？",
    type: "textarea",
    placeholder: "例如：TikTok Shop 上架、短视频脚本、达人邀约、数据复盘等",
  },
  {
    id: "task_source",
    label: "你的任务通常从哪里来？",
    type: "checkbox",
    options: ["老板安排", "主管分配", "运营计划", "产品上新", "数据反馈", "客户 / 达人反馈", "临时需求"],
  },
  {
    id: "needed_materials",
    label: "开始工作前通常需要哪些资料？",
    type: "textarea",
    placeholder: "例如：产品图、价格、竞品链接、关键词、历史脚本、达人资料等",
  },
  {
    id: "material_locations",
    label: "这些资料目前主要存在哪里？",
    type: "checkbox",
    options: TOOL_OPTIONS,
  },
  {
    id: "top_repeated_work",
    label: "每天 / 每周重复最多的工作是什么？",
    type: "textarea",
    placeholder: "请列出 1-5 项，越具体越好",
  },
  {
    id: "output_receiver",
    label: "你的工作结果通常交给谁？",
    type: "text",
    placeholder: "例如：运营主管、老板、剪辑、BD、平台运营",
  },
  {
    id: "reviewer",
    label: "谁会审核你的工作结果？",
    type: "text",
    placeholder: "例如：老板、部门主管、平台负责人；如果没人审核，也请写“无”",
  },
  {
    id: "time_consuming",
    label: "哪些步骤最耗时间？",
    type: "textarea",
    placeholder: "请说明耗时原因，例如资料不全、反复沟通、手动复制、需要多轮修改",
  },
  {
    id: "rework_causes",
    label: "哪些情况最容易出错或返工？",
    type: "textarea",
    placeholder: "例如：信息错、表达违规、素材不匹配、审核打回、达人回复慢",
  },
  {
    id: "current_templates",
    label: "是否已有固定模板、表格、话术或 SOP？",
    type: "radio",
    options: ["有，且经常使用", "有，但不完整", "没有，主要靠个人经验", "不清楚"],
  },
  {
    id: "records_saved",
    label: "完成工作后，过程和结果是否会被记录？",
    type: "radio",
    options: ["完整记录，后续会复用", "部分记录，但查找不方便", "很少记录", "基本不记录"],
  },
  {
    id: "ideal_dashboard",
    label: "如果有一个工作台页面，你希望打开后首先看到什么？",
    type: "textarea",
    placeholder: "例如：待处理任务、产品资料、AI 生成草稿、审核状态、达人跟进提醒、数据表现",
  },
];
...
export function getDepartmentLabel(id) {
  return DEPARTMENTS.find((department) => department.id === id)?.label || "未选择";
}

export function getQuestionsForDepartment(departmentId) {
  return {
...
export function buildEmptyResponse(departmentId = "") {
  return {
    meta: {
      name: "",
...
export function serializeResponse(response) {
  ...
  const departmentLabel = getDepartmentLabel(response.meta?.department);
  const stepCount = response.flow?.steps?.length || 0;
  const exportText = [
    `提交时间：${timestamp}`,
    `姓名：${response.meta?.name || ""}`,
    `部门：${departmentLabel}`,
    `岗位：${response.meta?.role || ""}`,
    `负责平台：${(response.meta?.platforms || []).join("、")}`,
    `流程步骤数：${stepCount}`,
    `AI 可辅助：${(response.ai?.usefulAreas || []).join("、")}`,
    `人工把控：${(response.ai?.humanOnly || []).join("、")}`,
    `资料链接：${response.attachments?.links || ""}`,
  ].join("\n");
  ...
  return {
    ...
    summary: {
      departmentLabel,
      stepCount,
      aiAreaCount: response.ai?.usefulAreas?.length || 0,
      humanOnlyCount: response.ai?.humanOnly?.length || 0,
    },
    exportText,
  };
}
